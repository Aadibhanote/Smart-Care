...
const validateDoctorAddData = (req) => {
  ...
  // ✅ Return array of errors or null if valid
  return errors.length > 0 ? errors : null;
};
...
const validateUserdata = (req) => {
  ...
  if (!firstName || !lastName) errors.push("Name fields are required.");
...
const validEditData = (req) => {
  const allowed = ["firstName", "lastName", "image", "address", "gender", "phone"];
  const keys = Object.keys(req.body);
  return keys.every((k) => allowed.includes(k));
};

// -------------------- DOCTOR EDIT VALIDATION --------------------
const validDocData = (req) => {
  const allowed = [
    "firstName",
    "lastName",
    "image",
